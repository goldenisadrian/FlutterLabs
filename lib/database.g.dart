    ...
    final databaseOptions = sqflite.OpenDatabaseOptions(
      ...
      onCreate: (database, version) async {
        await database.execute(
            'CREATE TABLE IF NOT EXISTS `items` (`id` INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, `name` TEXT NOT NULL, `quantity` INTEGER NOT NULL)');
...
class _$ItemDao extends ItemDao {
  ...
  Stream<Item?> findItemById(int id) {
    return _queryAdapter.queryStream('SELECT * FROM items WHERE id = ?1',
        mapper: (Map<String, Object?> row) => Item(
            row['id'] as int, row['name'] as String, row['quantity'] as int),
        arguments: [id],
        queryableName: 'items',
